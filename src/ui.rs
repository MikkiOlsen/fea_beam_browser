...
pub fn handle_input(state: &mut AppState) {
    ...
    let is_mouse_over_side_ui = (state.current_mode == AppMode::SetSupports || state.current_mode == AppMode::SetProperties || state.current_mode == AppMode::SetLoads) && mx > ui_x && my < 250.0;
    ...
    let mouse_pos = vec2(mx, my);
    if is_mouse_button_down(MouseButton::Middle) || (is_mouse_button_down(MouseButton::Left) && is_key_down(KeyCode::Space)) {
    ...
    if is_mouse_button_pressed(MouseButton::Left) {
        if my < 40.0 || is_mouse_over_side_ui { state.ui_focus = true; } else { state.ui_focus = false; }
    }
    ...
    if is_mouse_button_pressed(MouseButton::Left) && my < 40.0 {
        ...
        if mx < tab_w { state.current_mode = AppMode::PlaceNodes; }
        else if mx < tab_w * 2.0 { state.current_mode = AppMode::ConnectBeams; }
        else if mx < tab_w * 3.0 { state.current_mode = AppMode::SetSupports; }
        else if mx < tab_w * 4.0 { 
            state.current_mode = AppMode::SetProperties; 
            state.input_e = state.default_e.to_string(); state.input_a = state.default_a.to_string(); state.input_i = state.default_i.to_string();
        }
        else if mx < tab_w * 5.0 { state.current_mode = AppMode::SetLoads; }
        else { state.should_solve = true; }
        
        state.selected_node_id = None; state.selected_element_id = None;
        return; 
    }

    if !state.ui_focus {
        if is_key_pressed(KeyCode::Key1) { state.current_mode = AppMode::PlaceNodes; state.selected_node_id = None; state.selected_element_id = None; }
        if is_key_pressed(KeyCode::Key2) { state.current_mode = AppMode::ConnectBeams; state.selected_node_id = None; state.selected_element_id = None; }
        if is_key_pressed(KeyCode::Key3) { state.current_mode = AppMode::SetSupports; state.selected_node_id = None; state.selected_element_id = None; }
        if is_key_pressed(KeyCode::Key4) { 
            state.current_mode = AppMode::SetProperties; state.selected_element_id = None; 
            state.input_e = state.default_e.to_string(); state.input_a = state.default_a.to_string(); state.input_i = state.default_i.to_string();
        }
        if is_key_pressed(KeyCode::Key5) { state.current_mode = AppMode::SetLoads; state.selected_node_id = None; state.selected_element_id = None; }
    }

    let world_mouse = screen_to_world(mouse_pos, state.cam_zoom, state.cam_pan);
    let grid_size = 50.0;
    let snap_world = vec2((world_mouse.x / grid_size).round() * grid_size, (world_mouse.y / grid_size).round() * grid_size);
            ...
            for i in 0..4 {
                ...
                if my > btn_y - 15.0 && my < btn_y + 5.0 {
                    state.selected_support = match i {
                        0 => SupportType::Pin, 1 => SupportType::RollerH, 2 => SupportType::RollerV, _ => SupportType::Fixed,
                    };
                }
            }
        }
    } else {
        let hovered_idx = state.nodes.iter().position(|n| world_to_screen(n.pos, state.cam_zoom, state.cam_pan).distance(mouse_pos) < 15.0);

        if is_mouse_button_pressed(MouseButton::Left) {
            match state.current_mode {
                AppMode::PlaceNodes => {
                    if !state.nodes.iter().any(|n| n.pos.distance(snap_world) < 1.0) && my > 40.0 {
                        state.nodes.push(Node { id: state.next_node_id, pos: snap_world, support: SupportType::Free, fx: 0.0, fy: 0.0, m: 0.0 });
                ...
                AppMode::ConnectBeams => {
                    if let Some(idx) = hovered_idx {
                        ...
                        if let Some(first_id) = state.selected_node_id {
                            if first_id != clicked_id && !state.elements.iter().any(|e| (e.n1_id == first_id && e.n2_id == clicked_id) || (e.n1_id == clicked_id && e.n2_id == first_id)) {
                                state.elements.push(Element { 
                                    id: state.next_element_id, n1_id: first_id, n2_id: clicked_id,
                                    e: state.default_e, a: state.default_a, i: state.default_i, w: 0.0,
                                });
                                state.next_element_id += 1;
                                state.selected_node_id = None; 
                            }
                        } else { state.selected_node_id = Some(clicked_id); }
                    }
                }
                AppMode::SetSupports => {
                    if let Some(idx) = hovered_idx { state.nodes[idx].support = state.selected_support; }
                }
                AppMode::SetProperties => {
                    let mut closest = None; let mut min_dist = 15.0; 
                    for el in state.elements.iter() {
                        if let (Some(n1), Some(n2)) = (state.nodes.iter().find(|n| n.id == el.n1_id), state.nodes.iter().find(|n| n.id == el.n2_id)) {
                            let d = point_line_dist(mouse_pos, world_to_screen(n1.pos, state.cam_zoom, state.cam_pan), world_to_screen(n2.pos, state.cam_zoom, state.cam_pan));
                            if d < min_dist { min_dist = d; closest = Some(el.id); }
                        }
                    }
                    if let Some(id) = closest {
                        state.selected_element_id = Some(id);
                        let el = state.elements.iter().find(|e| e.id == id).unwrap();
                        state.input_e = el.e.to_string(); state.input_a = el.a.to_string(); state.input_i = el.i.to_string();
                    } else { state.selected_element_id = None; }
                }
                AppMode::SetLoads => {
                    if let Some(idx) = hovered_idx {
                        state.selected_node_id = Some(state.nodes[idx].id);
                        state.selected_element_id = None;
                        state.input_fx = state.nodes[idx].fx.to_string(); state.input_fy = state.nodes[idx].fy.to_string(); state.input_m = state.nodes[idx].m.to_string();
                    } else {
                        let mut closest = None; let mut min_dist = 15.0; 
                        for el in state.elements.iter() {
                            if let (Some(n1), Some(n2)) = (state.nodes.iter().find(|n| n.id == el.n1_id), state.nodes.iter().find(|n| n.id == el.n2_id)) {
            ...
            match state.current_mode {
                AppMode::PlaceNodes => {
                    if let Some(idx) = hovered_idx {
                        ...
                        state.elements.retain(|e| e.n1_id != del_id && e.n2_id != del_id);
                    }
                }
                AppMode::ConnectBeams => {
                    let mut closest = None; let mut min_dist = 15.0; 
                    for (i, el) in state.elements.iter().enumerate() {
                        if let (Some(n1), Some(n2)) = (state.nodes.iter().find(|n| n.id == el.n1_id), state.nodes.iter().find(|n| n.id == el.n2_id)) {
                            let d = point_line_dist(mouse_pos, world_to_screen(n1.pos, state.cam_zoom, state.cam_pan), world_to_screen(n2.pos, state.cam_zoom, state.cam_pan));
                            if d < min_dist { min_dist = d; closest = Some(i); }
                        }
                    }
                    if let Some(idx) = closest { state.elements.remove(idx); } else { state.selected_node_id = None; }
                }
                AppMode::SetSupports => { if let Some(idx) = hovered_idx { state.nodes[idx].support = SupportType::Free; } }
                AppMode::SetProperties => { state.selected_element_id = None; }
                AppMode::SetLoads => { state.selected_element_id = None; state.selected_node_id = None; }
            }
        }
    }
}
...
pub fn render(state: &mut AppState, fe: &crate::fe_manager::FeManager) { 
    ...
    let bottom_right_world = screen_to_world(vec2(screen_width(), screen_height()), state.cam_zoom, state.cam_pan);
    ...
    if (state.current_mode == AppMode::SetProperties || state.current_mode == AppMode::SetLoads) && !is_mouse_over_side_ui && my > 40.0 {
        let mut min_dist = 15.0; 
        for el in &state.elements {
            if let (Some(n1), Some(n2)) = (state.nodes.iter().find(|n| n.id == el.n1_id), state.nodes.iter().find(|n| n.id == el.n2_id)) {
                let d = point_line_dist(mouse_pos, world_to_screen(n1.pos, state.cam_zoom, state.cam_pan), world_to_screen(n2.pos, state.cam_zoom, state.cam_pan));
                if d < min_dist { min_dist = d; hovered_el_id = Some(el.id); }
            }
        }
    }

    let has_solution = fe.displacements.len() > 0 && fe.displacements.len() == state.nodes.len() * 3;
    ...
    if has_solution {
        for k in 0..state.nodes.len() {
            ...
            if mag > max_node_disp { max_node_disp = mag; }
        }
    }
    ...
    for el in &state.elements {
        if let (Some(i1), Some(i2)) = (
            ...
            let is_selected = (state.current_mode == AppMode::SetProperties || state.current_mode == AppMode::SetLoads) && state.selected_element_id == Some(el.id);
            let is_hovered = hovered_el_id == Some(el.id);
            let base_color = if is_selected { RED } else if is_hovered { GREEN } else { BLACK };

            let initial_thickness = if has_solution { 1.5 } else { if is_selected || is_hovered { 6.0 } else { 4.0 } };
            draw_line(sp1.x, sp1.y, sp2.x, sp2.y, initial_thickness * state.cam_zoom.sqrt(), base_color);
            ...
            if has_solution {
                ...
                if l_world > 0.0 {
                    ...
                    let u1_axial = ds1.dot(n_w); let v1_trans = ds1.dot(perp_w);
                    let u2_axial = ds2.dot(n_w); let v2_trans = ds2.dot(perp_w);
                    ...
                    for j in 0..ndata {
                        ...
                        let curr_screen_pt = world_to_screen(curr_world_pt, state.cam_zoom, state.cam_pan);

                        if j > 0 {
                            draw_line(prev_screen_pt.x, prev_screen_pt.y, curr_screen_pt.x, curr_screen_pt.y, 3.5 * state.cam_zoom.sqrt(), BLUE);
                        }
            ...
            if el.w != 0.0 {
                ...
                draw_line(top_p1.x, top_p1.y, top_p2.x, top_p2.y, 1.5 * state.cam_zoom.sqrt(), BLUE);
                ...
                for j in 0..=num_arrows {
                    ...
                    draw_arrow(top_pt, base_pt - load_dir * 4.0, BLUE, 1.5 * state.cam_zoom.sqrt()); 
                }
                
                let mid = p1.lerp(p2, 0.5) - load_dir * (offset + 15.0);
                draw_text(&format!("w = {} N/m", el.w), mid.x - 35.0, mid.y, 16.0, BLUE);
            }
        }
    }
    ...
    for (i, node) in state.nodes.iter().enumerate() {
        ...
        if node.fx != 0.0 {
            ...
            draw_text(&format!("{} N", node.fx), sp.x - 25.0, sp.y - 15.0, 14.0, RED);
        }
        ...
        if node.fy != 0.0 {
            ...
            draw_arrow(start, end, RED, 2.5);
            draw_text(&format!("{} N", node.fy), sp.x + 10.0, sp.y + 5.0, 14.0, RED);
        }
        ...
        if node.m != 0.0 {
            ...
            for j in 0..16 {
                ...
                draw_line(sp.x + t1.cos() * m_r, sp.y - t1.sin() * m_r, sp.x + t2.cos() * m_r, sp.y - t2.sin() * m_r, 2.5, PURPLE);
            }
            let tip = vec2(sp.x + end_a.cos() * m_r, sp.y - end_a.sin() * m_r);
            draw_arrow(tip + vec2(2.0, -4.0), tip, PURPLE, 2.5);
            draw_text(&format!("{} Nm", node.m), sp.x - 15.0, sp.y - (m_r + 6.0), 14.0, PURPLE);
        }
       ...
       let z = state.cam_zoom;
        match node.support {
            ...
            SupportType::Pin => {
                ...
                draw_line(sp.x - 18.0 * z, base_y, sp.x + 18.0 * z, base_y, 2.0 * z.sqrt(), RED);
            }
            SupportType::RollerH => {
                let r = 14.0 * z;
                let gap = 6.0 * z;
                ...
                draw_line(sp.x - 22.0 * z, floor_y, sp.x + 22.0 * z, floor_y, 2.0 * z.sqrt(), ORANGE);
            }
            SupportType::RollerV => {
                let r = 14.0 * z;
                let gap = 6.0 * z;
                ...
                draw_line(wall_x, sp.y - 22.0 * z, wall_x, sp.y + 22.0 * z, 2.0 * z.sqrt(), ORANGE);
            }
            SupportType::Fixed => {
                let gap = 6.0 * z;
                let base_y = sp.y + gap;
                ...
                for j in 0..=6 {
                    ...
                    draw_line(sp.x + x_offset, base_y, sp.x + x_offset - 8.0 * z, base_y + 12.0 * z, 1.5 * z.sqrt(), MAGENTA);
                }
            }
        }
    ...
    if !fe.solver_status.is_empty() {
        ...
        let msg = if is_error {
            ...
            format!("SOLVER SUCCESS [Deformation Scale: {:.1}x]", state.def_scale)
        };
    ...
    let tab_w = screen_width() / 6.0;
    let tabs = ["[1] NODES", "[2] BEAMS", "[3] SUPPORTS", "[4] PROPS", "[5] LOADS", "[ENT] SOLVE"];
    ...
    for (i, label) in tabs.iter().enumerate() {
        ...
        draw_text(label, x + (tab_w - text_size.width) / 2.0, 25.0, 20.0, BLACK);
    }
    ...
    if state.current_mode == AppMode::SetSupports {
        ...
        let options = [(SupportType::Pin, "Pin (Fixed X/Y)"), (SupportType::RollerH, "Roller (Fixed Y)"), (SupportType::RollerV, "Roller (Fixed X)"), (SupportType::Fixed, "Clamped (Fixed All)")];
        for (j, (s_type, label)) in options.iter().enumerate() {
            ...
            if mx > ui_x + 10.0 && mx < ui_x + 220.0 && my > btn_y - 15.0 && my < btn_y + 5.0 {
                draw_rectangle(ui_x + 10.0, btn_y - 15.0, 210.0, 20.0, Color::new(0.9, 0.9, 0.9, 1.0));
            }
            let prefix = if state.selected_support == *s_type { "[X]" } else { "[  ]" };
            draw_text(&format!("{} {}", prefix, label), ui_x + 15.0, btn_y, 20.0, BLACK);
        }
    }

    if state.current_mode == AppMode::SetProperties {
        let title = if state.selected_element_id.is_some() { "Element Properties" } else { "Global Defaults & View" };
        widgets::Window::new(hash!(), vec2(ui_x, 60.0), vec2(230.0, 230.0)).label(title).ui(&mut *root_ui(), |ui| {
            ...
            if ui.button(None, "Apply Changes") {
                ...
                if let Some(id) = state.selected_element_id {
                    if let Some(el) = state.elements.iter_mut().find(|e| e.id == id) {
                        el.e = parsed_e; el.a = parsed_a; el.i = parsed_i;
                    }
                } else {
                    state.default_e = parsed_e; state.default_a = parsed_a; state.default_i = parsed_i;
                }
            }
        });
    }
    ...
    if state.current_mode == AppMode::SetLoads {
        if state.selected_node_id.is_some() {
            widgets::Window::new(hash!(), vec2(ui_x, 60.0), vec2(230.0, 180.0)).label("Node Loads").ui(&mut *root_ui(), |ui| {
            ...
            widgets::Window::new(hash!(), vec2(ui_x, 60.0), vec2(230.0, 100.0)).label("Beam Uniform Load").ui(&mut *root_ui(), |ui| {
